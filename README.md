...
Journal entries use five markers, English or Russian: `DECIDED:` / `REJECTED:` / `RULE:` / `TAIL:` / `TERM:` (`РЕШЕНИЕ:` / `ОТКАЗ:` / `ПРАВИЛО:` / `ХВОСТ:` / `ТЕРМИН:`). Markers are seeds: the **harvest script** (run by the session-start hook) moves them into the registers with stable ids — a tail written once cannot be lost, it sits in `open.md` until explicitly closed. Operator-confirmed rules move up to `_principles.md`; nothing enters principles without the operator's word. Operator's verbatim words are never translated or trimmed. `INDEX.md` is generated by harvest — a view, never hand-edited.
